...
module.exports = function (collection, id, done) {
  ...
  Async.auto({
      ...
      getObject: ["validateCollection", "validateID", function (prev, next) {
        var collectionPath = Path.join(basePath, collection);
        internals.getObject(collectionPath, id, next);
      }]
      ...
      done(null, results.getObject);
    }
  );
}

internals.getObject = function (path, id, done) {
  ...
  Fs.readFile(documentPath, 'utf8', function (err, data) {
    ...
    try {
      ...
      if (err instanceof SyntaxError) {
        return done(new Error('document is not parsable'))
      }
